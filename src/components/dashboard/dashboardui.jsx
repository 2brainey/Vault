...
import { RenderIcon, getRarityGradient } from './dashboardutils'; // Import helper from sibling

// Go up two folders to find 'data'
import { 
  CARD_DATABASE, CONTRACTS_PER_PAGE, CARDS_PER_PAGE, 
  SKILL_DETAILS, INVENTORY_SLOTS 
} from '../../data/gamedata';

// --- SUB-COMPONENTS ---

export const WellnessBar = ({ label, value, iconName, onFill, color, task, tasks }) => {
      ...
      <div className="grid grid-cols-4 gap-2 overflow-y-auto pr-1 custom-scrollbar">
          {inventory?.map((item, i) => (
              <div 
                ...
                className={`aspect-square rounded border flex items-center justify-center text-slate-400 hover:text-white group relative transition-all bg-gradient-to-br cursor-pointer active:scale-95 ${getRarityGradient(item.rarity)}`}
              >
                  <RenderIcon name={item.iconName} size={20} />
                  {item.type === 'Box' && <div className="absolute top-1 right-1 text-[8px] bg-black/50 px-1 rounded text-amber-500 font-bold">BOX</div>}
                  {item.type === 'Pack' && <div className="absolute top-1 right-1 text-[8px] bg-black/50 px-1 rounded text-purple-400 font-bold">PK</div>}
                  ...
                  <div className="absolute bottom-full right-0 w-32 bg-black p-2 rounded border border-slate-700 text-xs hidden group-hover:block z-20 shadow-xl pointer-events-none">
                      ...
                      <div className="text-[9px] text-emerald-400 mt-1 italic">Click to Use</div>
                  </div>
              </div>
          ))}
...
export const ContractWidget = ({ contracts, onToggle, title = "Contracts" }) => {
