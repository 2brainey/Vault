...
export default function VaultDashboard() {
  ...
  return (
    <div className="min-h-screen text-slate-200 font-sans bg-vault-dark flex flex-col">
      ...
      {/* --- UPDATED HEADER (Removed Vitals & Resources from here) --- */}
      <div className="sticky top-0 z-[60] bg-[#2b3446] border-b border-[#404e6d] shadow-xl">
          <header className="w-full max-w-7xl mx-auto p-3 flex justify-between items-center">
             <div className="flex items-center gap-4 cursor-pointer" onClick={() => setActiveTab('dynamic')}>
                ...
                <div>
                    ...
                    <div className="text-[10px] text-slate-400 font-mono mt-1 uppercase tracking-widest">Architect Terminal</div>
                </div>
             </div>
             
             {/* Center: Quick Stats (Passive/Active) */}
          ...
          {/* Removed the Vitals Footer from here */}
      </div>
                ...
                <div className={`bg-[#1a1a1a] border-l border-slate-700 flex flex-col shrink-0 relative transition-all duration-200 z-[70] ${isRightCollapsed ? 'w-14 items-center py-4' : ''}`} style={{ width: isRightCollapsed ? '3.5rem' : `${rightPanelWidth}px` }}>
                    ...
                    {isRightCollapsed ? (
                        <div className="flex flex-col gap-4 items-center w-full">
                            ...
                            <div className="w-8 h-px bg-slate-700"></div>
                            {/* Collapsed Icons for Resources */}
                            <div className="flex flex-col gap-2 items-center">
                                <div className="p-2 rounded bg-slate-800 text-emerald-400" title={`$${data.cash}`}><RenderIcon name="DollarSign" size={16}/></div>
                                <div className="p-2 rounded bg-slate-800 text-pink-400" title={`${data.discipline} BM`}><RenderIcon name="Brain" size={16}/></div>
                                <div className="p-2 rounded bg-slate-800 text-slate-400" title={`${data.salvage} Salvage`}><RenderIcon name="Wrench" size={16}/></div>
                            </div>
                            <div className="w-8 h-px bg-slate-700"></div>
                        ...
                        <div className={`flex-1 flex flex-col overflow-hidden opacity-100 w-full`}>
                            {/* NEW: RESOURCE CARD & VITALS HUD AT TOP OF RIGHT SIDEBAR */}
                            <div className="p-4 pb-0 space-y-2 overflow-y-auto shrink-0 max-h-[50%]">
                                <ResourceCard cash={data.cash} discipline={data.discipline} salvage={data.salvage} />
                                <VitalsHUD wellness={data.wellness} onMaintain={handleMaintainVital} />
                            </div>

                            <div className="flex items-center justify-between p-3 border-b border-slate-800 bg-[#131313] mt-2">
