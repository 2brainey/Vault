...
import { RenderIcon } from './dashboardutils'; // Assuming this provides the helper

// --- DATA CONSTANTS (Self-Contained for Preview) ---
...
export default function EstatePrototype({ discipline: parentDiscipline = 5000, setDiscipline: parentSetDiscipline = () => {} }) {
  ...
  // 1. Local State Fallback (For isolated testing)
  ...
  const [bonusView, setBonusView] = useState('card'); 
  ...
  const handleSelectShopItem = (item) => {
      if (discipline < item.cost) {
          alert("Insufficient Discipline!");
          return;
      }

      // 1. Handle Expansion
      if (item.isExpansion) {
          if (gridDimension >= MAX_GRID_DIMENSION) {
              alert("Maximum Estate size reached!");
              return;
          }
          expandGrid();
      ...
      // 2. Set Pending Item (Building or Deed)
      setPendingBuildItem(item);
      setSidebarOpen(false); 
      
      // Auto-action if valid slot selected
      if (selectedSlot !== null) {
          if (item.isDeed && grid[selectedSlot] === null) {
              buyPlot(selectedSlot);
          }
          else if (!item.isDeed && grid[selectedSlot] === 'empty') {
              finalizeBuild(selectedSlot, item);
          }
      }
  };
  ...
  const handleSlotClick = (index) => {
      ...
      // 1. Buy Land
      if (isUnowned) {
          if (pendingBuildItem?.isDeed) {
              buyPlot(index);
              return;
          }
          
          if (pendingBuildItem && !pendingBuildItem.isDeed) {
              alert("You must own this land before building!");
              return;
          }
      ...
      if (pendingBuildItem) {
          if (pendingBuildItem.isDeed) {
              alert("You already own this plot.");
              return;
          }
  ...
  const buyPlot = (index) => {
      ...
      newGrid[index] = 'empty';
      setGrid(newGrid);
      setDiscipline(d => d - PLOT_COST);
      setSelectedSlot(null);
  };

  const finalizeBuild = (index, item) => {
      const newGrid = [...grid];
      newGrid[index] = { ...item, builtAt: Date.now() };
      setGrid(newGrid);
      setDiscipline(d => d - item.cost);
      setPendingBuildItem(null);
      setSelectedSlot(null);
  ...
  const filteredBonuses = useMemo(() => {
      return ESTATE_ITEMS.filter(item => !item.isExpansion && !item.isBundle && !item.isDeed).filter(room => {
          const isOwned = roomCounts[room.id] > 0;
          const filterLogic = (f) => {
              if (f === 'owned') return isOwned;
              if (f === 'locked') return !isOwned;
              return true;
          };
          return filterLogic(bonusFilter);
      });
  }, [bonusFilter, roomCounts]);
  ...
  return (
    <div className="h-full flex gap-0 animate-in fade-in overflow-hidden relative p-0">
      ...
      <div className={`flex-1 bg-[#1a202c] border-2 border-slate-700 rounded-xl p-6 relative overflow-hidden shadow-2xl flex flex-col transition-all duration-300 ${isSidebarOpen ? 'rounded-r-none border-r-0' : ''}`}>
        ...
        {/* Refined Background Pattern: Smaller 10px Grid for "Graph Paper" effect */}
        ...
        <div className="relative z-10 flex justify-between items-center px-6 pt-6 pb-4 shrink-0">
            <div>
                ...
                <div className="h-6 flex items-center">
                    {pendingBuildItem ? (
                         <span className="text-amber-400 text-sm font-bold animate-pulse flex items-center gap-2 bg-black/40 px-2 rounded">
            ...
            <div className="flex gap-2">
                 <button onClick={() => setDiscipline(99999999)} className="bg-emerald-900/30 hover:bg-emerald-900/50 text-emerald-400 p-2 rounded-lg border border-emerald-800 transition-colors" title="Add Debug Funds"><RenderIcon name="Coins" size={18}/></button>

                 {!isSidebarOpen && (
                    <button onClick={() => setSidebarOpen(true)} className="bg-slate-700 hover:bg-slate-600 text-white p-2 rounded-lg border border-slate-500 transition-colors flex items-center gap-2">
                        <RenderIcon name="PanelRightOpen" size={18}/> <span className="text-xs font-bold">Open Shop</span>
                    </button>
                 )}
        ...
        <div className="relative z-10 flex-1 flex items-center justify-center overflow-auto p-4">
            <div 
                ...
                style={{ 
                    ...
                    width: '100%',
                    maxWidth: isSidebarOpen ? '500px' : '700px', 
                    aspectRatio: '1/1'
                }}
            >
                {safeGrid.map((plot, i) => {
                    ...
                    return (
                        <div 
                            ...
                            className={`
                                ...
                                ${isBuilt 
                                    ? 'bg-slate-800 border-[#e1b542] shadow-lg z-10 shadow-[#e1b542]/20' // GOLD BORDER FOR BUILT
                                    : isOwned 
                                        ? pendingBuildItem 
                                            ? (pendingBuildItem.isDeed ? 'bg-red-900/20 border-red-500/30' : 'bg-emerald-900/30 border-emerald-500/50 animate-pulse')
                                            : 'bg-white/5 border-[#e1b542]/50 hover:bg-white/10 hover:border-[#e1b542]' // GOLD BORDER FOR OWNED EMPTY
                        ...
                        >
                            {isBuilt ? (
                                ...
                                <div className={`flex flex-col items-center ${pendingBuildItem && !pendingBuildItem.isDeed ? 'text-emerald-400' : 'text-[#e1b542]/50 group-hover:text-[#e1b542]'}`}>
      ...
      <div className={`
            flex flex-col gap-0 h-full overflow-hidden transition-all duration-300 bg-[#1e1e1e] border-l border-slate-700 shadow-2xl
            ${isSidebarOpen ? 'w-[400px] translate-x-0 rounded-l-none border-2' : 'w-0 translate-x-full opacity-0 pointer-events-none border-0'}
      `}>
          
          <div className="p-4 border-b border-slate-700 bg-[#131313] shrink-0 flex flex-col gap-4">
             <div className="flex justify-between items-center">
                 <h3 className="font-bold text-white flex items-center gap-2"><RenderIcon name="Construction" size={16} className="text-amber-500"/> Estate Management</h3>
                 <button onClick={() => setSidebarOpen(false)} className="text-slate-400 hover:text-white"><RenderIcon name="PanelRightClose" size={18}/></button>
             </div>
             
             <div className="flex bg-black/30 p-1 rounded-lg">
                 <button onClick={() => setSidebarTab('shop')} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-colors flex items-center justify-center gap-2 ${sidebarTab === 'shop' ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}><RenderIcon name="Hammer" size={12}/> Shop</button>
                 <button onClick={() => setSidebarTab('bonuses')} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-colors flex items-center justify-center gap-2 ${sidebarTab === 'bonuses' ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}><RenderIcon name="ArrowUpRight" size={12}/> Bonuses</button>
             </div>
             
             <div className="flex justify-between items-center text-xs">
                <span className="text-slate-500 uppercase font-bold">Balance</span>
                ...
                <div className="grid grid-cols-2 gap-3">
                    {ESTATE_ITEMS.map(item => (
                        <button 
                            ...
                            className={`relative p-3 rounded-lg border flex flex-col items-center text-center gap-2 transition-all group ${pendingBuildItem?.id === item.id ? 'ring-2 ring-amber-500 bg-slate-800' : ''} ${discipline >= item.cost ? 'bg-slate-800/50 border-slate-700 hover:border-amber-500 hover:bg-slate-800' : 'bg-slate-900/50 border-slate-800 opacity-50 cursor-not-allowed'}`}
                        >
                            {item.isExpansion && <div className="absolute top-1 right-1 text-[8px] bg-purple-900/80 text-purple-200 px-1 rounded uppercase font-bold">Expand</div>}
                            {item.isBundle && <div className="absolute top-1 right-1 text-[8px] bg-blue-900/80 text-blue-200 px-1 rounded uppercase font-bold">Bundle</div>}
                            {item.isDeed && <div className="absolute top-1 right-1 text-[8px] bg-emerald-900/80 text-emerald-200 px-1 rounded uppercase font-bold">Land</div>}
                            
                            <div className={`p-2 rounded-full bg-black/30 ${discipline >= item.cost ? 'text-amber-500 group-hover:text-white' : 'text-slate-600'}`}><RenderIcon name={item.icon} size={20}/></div>
                            <div className="w-full"><div className="text-[10px] font-bold text-white truncate">{item.name}</div><div className="text-[9px] text-slate-500 font-mono">{item.cost.toLocaleString()}</div></div>
                            
                            {/* Hover Tooltip */}
                            <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center p-2 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg backdrop-blur-sm pointer-events-none">
                                <p className="text-[10px] text-slate-300 leading-tight mb-2">{item.desc}</p>
                                <span className="text-[9px] font-bold text-emerald-400">Click to Build</span>
                            </div>
                        </button>
                    ))}
                </div>
             ...
             {sidebarTab === 'bonuses' && (
                 <div className="space-y-2">
                     {filteredBonuses.length > 0 ? (
                        filteredBonuses.map(room => {
                           const count = roomCounts[room.id];
                           return (
                              <div key={room.id} className="p-3 rounded border border-slate-700 bg-slate-800/50 flex justify-between items-center">
                                  <div className="flex items-center gap-3"><div className="p-1.5 bg-black/30 rounded text-blue-400"><RenderIcon name={room.icon} size={16}/></div><div><div className="text-xs font-bold text-white">{room.name}</div><div className="text-[9px] text-slate-400">{room.desc}</div></div></div>
                                  <div className="text-xs font-mono font-bold text-emerald-400">x{count}</div>
                              </div>
                           )
                        })
                     ) : <div className="text-center p-8 text-slate-500 text-xs italic border border-dashed border-slate-700 rounded-lg">No active facilities.</div>}
                 </div>
             )}
          </div>
      </div>
